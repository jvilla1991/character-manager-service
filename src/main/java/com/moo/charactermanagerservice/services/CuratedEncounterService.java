 ...
 * campaign's DM and DM-only actions assert it (403 otherwise). Loot is no longer
 * attached here — prepped spoils are standalone {@link CuratedLootService} lists.
 */
@Service
public class CuratedEncounterService {
    ...
    /** Add a creature line to the encounter. AC is optional (null = unknown). */
    @Transactional
    public EncounterView addCreature(Long encounterId, String name, Short armorClass,
                                     Short hpMax, Integer quantity, UUID dmUserId) {
        Encounter encounter = requireOwnedEncounter(encounterId, dmUserId);
        validateCreature(name, quantity);
    ...
    public EncounterView updateCreature(Long encounterId, Long creatureId, String name,
                                        Short armorClass, Short hpMax, Integer quantity, UUID dmUserId) {
        Encounter encounter = requireOwnedEncounter(encounterId, dmUserId);
        validateCreature(name, quantity);
    ...
    private EncounterView buildView(Encounter encounter) {
        List<EncounterCreatureView> creatures = creatureRepository
                ...
                .map(c -> new EncounterCreatureView(c.getId(), c.getName(),
                        c.getArmorClass(), c.getHpMax(), c.getQuantity()))
                .toList();
    ...
    private void validateCreature(String name, Integer quantity) {
        if (name == null || name.isBlank()) {
